...
public class EventImportStep : IRecipeStepHandler
{
  ...
  public async Task ExecuteAsync(RecipeExecutionContext context)
  {
    ...
    if (streams is not JArray) return;

    foreach (var stream in streams.Cast<JObject>())
    {
      var aggregateTypeName = (string?)(
        stream.Property("AggregateType")?.Value as JValue
      );
      if (aggregateTypeName is null) continue;
      ...
      var events = (stream.Property("Events")?.Value as JArray)
        ?.Select(
          @event =>
          {
            if (@event is not JObject eventObject) return null;

            var eventTypeName = (string?)(
              eventObject.Property("EventType")?.Value as JValue
            );
            if (eventTypeName is null) return null;
            ...
            return eventData is not JObject
              ? null
              : eventData.ToObject(eventType);
          }
        )
        .Cast<IEvent>()
