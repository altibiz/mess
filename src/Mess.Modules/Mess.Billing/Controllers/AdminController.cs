...
public class AdminController : Controller
{
  ...
  {
    var invoiceItem = await _contentManager.GetContentAsync<InvoiceItem>(contentItemId);
    if (invoiceItem == null) {
      return NotFound();
    }
    ...
    var billableItem = await _contentManager.GetAsync(invoiceItem.InvoicePart.Value.Invoice.BillableContnetItemId);
    if (billableItem == null) {
      return NotFound();
    }

    var receiptFactory =_serviceProvider.GetServices<IReceiptFactory>()
      .Where(factory => factory.ContentType == billableItem.ContentType)
      .FirstOrDefault();
    if (receiptFactory == null) {
      throw new NotImplementedException($"No receipt factory for {billableItem.ContentType}");
    }
